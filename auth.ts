...
import {
  supabase,
  submissionToDb, dbToSubmission,
  scheduleToDb, dbToSchedule,
  noticeToDb, dbToNotice,
  messageToDb, dbToMessage,
  reportToDb, dbToReport,
} from "./supabase";
...
import ManagerCredentialsPanel from "./ManagerCredentialsPanel";
import RoutinesAgendaPanel from "./RoutinesAgendaPanel";
import ManagerWarningsPanel from "./ManagerWarningsPanel";
...
import { updatePassword, getStoredCredentials } from "./auth";
...
export default function App() {
  ...
  // Save collaboratorMessages → Supabase
  const prevMsgsRef = typeof useRef !== "undefined" ? useRef<CollaboratorMessage[]>([]) : { current: [] as CollaboratorMessage[] };
  useEffect(() => {
    const prev = prevMsgsRef.current;
    prev.forEach(m => {
      if (!collaboratorMessages.find(c => c.id === m.id)) {
        supabase.from("collaborator_messages").delete().eq("id", m.id);
      }
    });
    collaboratorMessages.forEach(m => {
      const old = prev.find(c => c.id === m.id);
      if (!old || JSON.stringify(old) !== JSON.stringify(m)) {
        supabase.from("collaborator_messages").upsert(messageToDb(m));
      }
    });
    prevMsgsRef.current = collaboratorMessages;
    try { localStorage.setItem("survey_tour_collaborator_msg", JSON.stringify(collaboratorMessages)); } catch {}
  ...
  // Save tourSchedules → Supabase (upsert entire list on change)
  const prevSchedulesRef = typeof useRef !== "undefined" ? useRef<TourSchedule[]>([]) : { current: [] as TourSchedule[] };
  useEffect(() => {
    const prev = prevSchedulesRef.current;
    // Find deleted
    prev.forEach(s => {
      if (!tourSchedules.find(t => t.id === s.id)) {
        supabase.from("tour_schedules").delete().eq("id", s.id);
      }
    });
    // Upsert new/changed
    tourSchedules.forEach(s => {
      const old = prev.find(t => t.id === s.id);
      if (!old || JSON.stringify(old) !== JSON.stringify(s)) {
        supabase.from("tour_schedules").upsert(scheduleToDb(s));
      }
    });
    prevSchedulesRef.current = tourSchedules;
    try { localStorage.setItem("survey_tour_agenda", JSON.stringify(tourSchedules)); } catch {}
  }, [tourSchedules]);

  // Save managerNotices → Supabase
  const prevNoticesRef = typeof useRef !== "undefined" ? useRef<ManagerNotice[]>([]) : { current: [] as ManagerNotice[] };
  useEffect(() => {
    const prev = prevNoticesRef.current;
    prev.forEach(n => {
      if (!managerNotices.find(m => m.id === n.id)) {
        supabase.from("manager_notices").delete().eq("id", n.id);
      }
    });
    managerNotices.forEach(n => {
      const old = prev.find(m => m.id === n.id);
      if (!old || JSON.stringify(old) !== JSON.stringify(n)) {
        supabase.from("manager_notices").upsert(noticeToDb(n));
      }
    });
    prevNoticesRef.current = managerNotices;
    try { localStorage.setItem("survey_tour_notices", JSON.stringify(managerNotices)); } catch {}
  }, [managerNotices]);
  ...
  // Save dailyReports → Supabase
  const prevReportsRef = typeof useRef !== "undefined" ? useRef<DailyActivityReport[]>([]) : { current: [] as DailyActivityReport[] };
  useEffect(() => {
    const prev = prevReportsRef.current;
    prev.forEach(r => {
      if (!dailyReports.find(d => d.id === r.id)) {
        supabase.from("daily_reports").delete().eq("id", r.id);
      }
    });
    dailyReports.forEach(r => {
      const old = prev.find(d => d.id === r.id);
      if (!old || JSON.stringify(old) !== JSON.stringify(r)) {
        supabase.from("daily_reports").upsert(reportToDb(r));
      }
    });
    prevReportsRef.current = dailyReports;
    try { localStorage.setItem("survey_tour_daily_reports", JSON.stringify(dailyReports)); } catch {}
  }, [dailyReports]);
  ...
  // ─── SUPABASE: Load submissions + Realtime ──────────────────
  useEffect(() => {
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          try {
            ...
            if (stored) {
              const parsed: SurveySubmission[] = JSON.parse(stored).filter(
                (s: SurveySubmission) => s && s.id && !s.id.startsWith("sub-")
              );
        ...
        const subs = data.map(dbToSubmission);
        setSubmissions(subs);
        try { localStorage.setItem("survey_tour_submissions", JSON.stringify(subs)); } catch {}
      });

    const channel = supabase
      .channel("submissions-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "survey_submissions" },
        (payload) => {
          const newSub = dbToSubmission(payload.new as Record<string, unknown>);
          setSubmissions(prev => {
            if (prev.some(s => s.id === newSub.id)) return prev;
            const updated = [newSub, ...prev];
            try { localStorage.setItem("survey_tour_submissions", JSON.stringify(updated)); } catch {}
            return updated;
          });
        })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "survey_submissions" },
        (payload) => {
          setSubmissions(prev => {
            const updated = prev.filter(s => s.id !== (payload.old as Record<string, unknown>).id);
            try { localStorage.setItem("survey_tour_submissions", JSON.stringify(updated)); } catch {}
            return updated;
          });
        })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "survey_submissions" },
        (payload) => {
          const upd = dbToSubmission(payload.new as Record<string, unknown>);
          setSubmissions(prev => {
  ...
  // Add new survey submission → Supabase
  const handleAddSubmission = async (newSub: SurveySubmission) => {
    if (newSub.liderEducador) handleAddLeader(newSub.liderEducador);
    if (newSub.produto) handleAddProduct(newSub.produto);
    await supabase.from("survey_submissions").insert(submissionToDb(newSub));
    setTimeout(() => { setActiveTab("dashboard"); }, 1500);
  };

  // Add sample → Supabase
  const handleAddSample = async (sample: SurveySubmission) => {
    if (sample.liderEducador) handleAddLeader(sample.liderEducador);
    if (sample.produto) handleAddProduct(sample.produto);
    await supabase.from("survey_submissions").insert(submissionToDb(sample));
  };

  // Delete submission → Supabase
  const handleDeleteSubmission = async (id: string) => {
    await supabase.from("survey_submissions").delete().eq("id", id);
  };

  // Update submission → Supabase
  const handleUpdateSubmission = async (updatedRow: SurveySubmission) => {
    ...
    await supabase.from("survey_submissions").upsert(submissionToDb(updatedRow));
  };
